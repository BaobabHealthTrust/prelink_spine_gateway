class Spine::Person < ActiveRecord::Base
  ...
  has_one :patient, :class_name => 'Spine::Patient', :foreign_key => :patient_id, :dependent => :destroy
  has_many :names, :class_name => 'Spine::PersonName', :foreign_key => :person_id, :dependent => :destroy, :conditions => 'person_name.voided = 0', :order => 'person_name.preferred DESC'
  # has_many :addresses, :class_name => 'Spine::PersonAddress', :foreign_key => :person_id, :dependent => :destroy, :conditions => 'person_address.voided = 0', :order => 'person_address.preferred DESC'
  # has_many :observations, :class_name => 'Spine::Observation', :foreign_key => :person_id, :dependent => :destroy, :conditions => 'obs.voided = 0' do
  ...
  def self.search_by_identifier(identifier)
    Spine::PatientIdentifier.find_all_by_identifier(identifier).map{|id| id.patient.person} unless identifier.blank?
  end
